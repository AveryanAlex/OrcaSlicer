...
constexpr const char* ORCA_SUBSCRIBED_PLUGINS  = "/api/v1/plugins/subscriptions";
constexpr const char* ORCA_UNSUBSCRIBE_PLUGINS = "/api/v1/plugins/subscriptions";
...
long long get_json_int64_field(const json& j, const std::string& key)
{
    if (!j.contains(key))
        return 0;
...
std::string resolve_display_name(const std::string& display_name,
...
std::string os_machine_id()
{
    ...
    if (RegOpenKeyExW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Microsoft\\Cryptography", 0, KEY_READ | KEY_WOW64_64KEY, &hkey) == ERROR_SUCCESS) {
        ...
        DWORD type          = 0;
        LONG status         = RegQueryValueExW(hkey, L"MachineGuid", nullptr, &type, reinterpret_cast<LPBYTE>(buffer), &buffer_bytes);
        RegCloseKey(hkey);
...
std::string hmac_sha256_hex(const std::string& data, const std::vector<unsigned char>& key)
{
    ...
    if (HMAC(EVP_sha256(), key.data(), static_cast<int>(key.size()), reinterpret_cast<const unsigned char*>(data.data()), data.size(),
             result, &len) == nullptr) {
        return {};
    }
...
bool aes256gcm_encrypt(const std::string& plaintext, const std::vector<unsigned char>& key, std::string& out_b64)
{
    ...
    if (EVP_EncryptInit_ex(ctx, EVP_aes_256_gcm(), nullptr, nullptr, nullptr) != 1)
        ok = false;
    if (ok && EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, iv_len, nullptr) != 1)
        ok = false;
    if (ok && EVP_EncryptInit_ex(ctx, nullptr, nullptr, key.data(), iv.data()) != 1)
        ok = false;
    if (ok &&
        EVP_EncryptUpdate(ctx, ciphertext.data(), &len, reinterpret_cast<const unsigned char*>(plaintext.data()), plaintext.size()) != 1)
        ok = false;
    int ciphertext_len = len;
    if (ok && EVP_EncryptFinal_ex(ctx, ciphertext.data() + len, &len) != 1)
        ok = false;
    ciphertext_len += len;
    if (ok && EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, tag.size(), tag.data()) != 1)
        ok = false;
...
{
    std::vector<unsigned char> payload;
    if (!base64url_decode(b64_payload, payload))
        return false;
    ...
    if (ok && EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, iv_len, nullptr) != 1)
        ok = false;
    if (ok && EVP_DecryptInit_ex(ctx, nullptr, nullptr, key.data(), iv.data()) != 1)
        ok = false;
    if (ok && EVP_DecryptUpdate(ctx, plain.data(), &len, ciphertext.data(), ciphertext.size()) != 1)
        ok = false;
    int plain_len = len;
    if (ok && EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, tag.size(), tag.data()) != 1)
        ok = false;
...
void OrcaCloudServiceAgent::set_auth_base_url(const std::string& url) { auth_base_url = url; }

void OrcaCloudServiceAgent::set_cloud_base_url(const std::string& url) { cloud_base_url = url; }

void OrcaCloudServiceAgent::set_use_encrypted_token_file(bool use) { m_use_encrypted_token_file = use; }

bool OrcaCloudServiceAgent::get_use_encrypted_token_file() const { return m_use_encrypted_token_file; }

// ============================================================================
// ICloudServiceAgent - Lifecycle Methods
// ============================================================================

int OrcaCloudServiceAgent::init_log() { return BAMBU_NETWORK_SUCCESS; }
...
int OrcaCloudServiceAgent::change_user(std::string user_info)
{
    try {
        ...
        if (command == "user_login") {
            ...
            BOOST_LOG_TRIVIAL(info) << "[auth] event=login result=" << (success ? "success" : "failure") << " source=webview";
...
int OrcaCloudServiceAgent::user_logout(bool request)
{
    ...
    if (request) {
        ...
        if (!token.empty()) {
            ...
            int result = http_post_auth(auth_constants::LOGOUT_PATH, logout_req.dump(), &response, &http_code) ?
...
std::string OrcaCloudServiceAgent::build_logout_cmd() { return json{{"command", "orca_useroffline"}}.dump(); }
...
bool OrcaCloudServiceAgent::ensure_token_fresh(const std::string& reason) { return refresh_if_expiring(TOKEN_REFRESH_SKEW, reason); }
...
std::string OrcaCloudServiceAgent::request_setting_id(std::string name,
...
int OrcaCloudServiceAgent::sync_pull(std::function<void(const SyncPullResponse&)> on_success,
                                                ...
                                                bool force)
{
    ...
    if (body_str.size() > ORCA_SYNC_MAX_PAYLOAD_SIZE) {
        ...
        result.success       = false;
        result.error_message = "Preset content exceeds 1MB size limit (actual: " + std::to_string(body_str.size()) + " bytes)";
        BOOST_LOG_TRIVIAL(error) << "OrcaCloudServiceAgent: sync_push payload too large - " << "size=" << body_str.size() << " bytes, "
                                 << "limit=" << ORCA_SYNC_MAX_PAYLOAD_SIZE << " bytes, " << "profile_id=" << profile_id;
        return result;
    }
...
void OrcaCloudServiceAgent::set_session_handler(SessionHandler handler) { session_handler = std::move(handler); }
...
bool OrcaCloudServiceAgent::should_refresh_locked(std::chrono::seconds skew) const
{
    if (!session.logged_in)
        return false;
    if (session.expires_at.time_since_epoch().count() == 0)
        return true;
...
bool OrcaCloudServiceAgent::decode_jwt_expiry(const std::string& token, std::chrono::system_clock::time_point& out_tp)
{
    ...
    try {
        ...
        if (payload.contains("exp") && payload["exp"].is_number()) {
            out_tp = std::chrono::system_clock::time_point{std::chrono::seconds(payload["exp"].get<long long>())};
            return true;
        }
    } catch (const std::exception& e) {
...
bool OrcaCloudServiceAgent::set_user_session(const json& session_json, bool notify_login)
{
    ...
    if (session_json.contains("user") && session_json["user"].is_object()) {
        ...
        nickname = resolve_display_name(get_json_string_field(session_json, "display_name"),
                                        get_json_string_field(session_json, "nickname"), get_json_string_field(session_json, "full_name"),
                                        get_json_string_field(session_json, "name"), username);
...
bool OrcaCloudServiceAgent::http_post_token(const std::string& body,
...
bool OrcaCloudServiceAgent::http_post_auth(const std::string& path,
...
void OrcaCloudServiceAgent::invoke_server_connected_callback(int return_code, int reason_code)
{
    ...
    if (callback) {
        ...
        if (queue_fn) {
            queue_fn([callback, event, return_code, reason_code]() { callback(event, return_code, reason_code); });
        } else {
...
void OrcaCloudServiceAgent::invoke_http_error_callback(unsigned http_code, const std::string& http_body)
{
    ...
    if (callback) {
        CloudEvent event{ORCA_CLOUD_PROVIDER};
        if (queue_fn) {
            queue_fn([callback, event, http_code, http_body]() { callback(event, http_code, http_body); });
        } else {
                                                  ...
                                                  std::vector<std::string>& unauthorized)
{
    ...
    if (result != 0 || http_code != 200) {
        BOOST_LOG_TRIVIAL(error) << "get_subscribed_bundles failed: http_code=" << http_code << ", response=" << response_body;
...
int OrcaCloudServiceAgent::get_shared_bundle(const std::string& bundle_id,
                                             ...
                                             BundleMetadata* bundle_metadata)
{
    ...
    if (result != 0 || http_code != 200) {
        BOOST_LOG_TRIVIAL(error) << "get_shared_bundle failed: bundle_id=" << bundle_id << ", http_code=" << http_code;
        return result != 0 ? result : http_code;
    }
    ...
    try {
        ...
        if (json.contains("id"))
            bundle_metadata->id = json["id"].get<std::string>();
        if (json.contains("name"))
            bundle_metadata->name = json["name"].get<std::string>();
        if (json.contains("version"))
            bundle_metadata->version = json["version"].get<std::string>();
        if (json.contains("description"))
            bundle_metadata->description = json["description"].get<std::string>();
        if (json.contains("author"))
            bundle_metadata->author = json["author"].get<std::string>();
        if (json.contains("updated_time"))
            bundle_metadata->updated_time = json["updated_time"].get<long long>();
        ...
        BOOST_LOG_TRIVIAL(info) << "get_shared_bundle: loaded " << presets->size() << " presets for bundle_id=" << bundle_id;
        return 0;
...
// Parse the cloud manifest "type" field (single string or array of strings) into a list of raw
// display labels, kept verbatim and NOT mapped to PluginCapabilityType: they are a purely cosmetic
// "compatibility" taxonomy (e.g. "printer-agent") shown like tags, and may not reflect the plugin's
// real capabilities (which are known only once the plugin is loaded). Prefers "type_label", then
// "type"; skips empty labels and de-duplicates.
static std::vector<std::string> parse_cloud_display_types(const nlohmann::json& item)
{
    std::vector<std::string> labels;
    auto add = [&labels](std::string label) {
        if (!label.empty() && std::find(labels.begin(), labels.end(), label) == labels.end())
            labels.push_back(std::move(label));
    };
    for (const char* key : {"type_label", "type"}) {
        ...
        if (field.is_array()) {
            ...
            add(field.get<std::string>());
        }
        if (!labels.empty())
            break; // "type_label" wins if present; else fall back to "type"
    }
    return labels;
}

// True when a cloud main_image is a still/animated image the WebView <img> can render
// (PNG/JPEG/GIF/WebP...). Other media (e.g. video) is skipped so no thumbnail is shown.
static bool cloud_media_is_image(const nlohmann::json& main_image)
{
    ...
    if (lowered(get_json_string_field(main_image, "mime_type")).rfind("image/", 0) == 0)
        return true;
                                                            ...
                                                            std::vector<std::string>& unauthorized)
{
    ...
    const std::string path = std::string(ORCA_SUBSCRIBED_PLUGINS) + "?include=changelog";
    int result = http_get(path, &response_body, &http_code);

    if (result != 0 || http_code != 200) {
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << " failed: http_code=" << http_code;
        return result != 0 ? result : http_code;
    }

    try {
        BOOST_LOG_TRIVIAL(info) << response_body;
        auto root = json::parse(response_body);

        if (root.contains("not_found") && root["not_found"].is_array()) {
            for (const auto& item : root["not_found"])
                not_found.push_back(item.get<std::string>());
        }
        if (root.contains("unauthorised") && root["unauthorised"].is_array()) {
            for (const auto& item : root["unauthorised"])
                unauthorized.push_back(item.get<std::string>());
        }

        if (!root.contains("data") || !root["data"].is_array()) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": invalid response format, missing data array";
            return -1;
        }

        for (const auto& item : root["data"]) {
            PluginDescriptor descriptor;
            std::string uuid = get_json_string_field(item, "uuid");
            if (uuid.empty())
                uuid = get_json_string_field(item, "id");

            descriptor.name        = sanitize_plugin_name(get_json_string_field(item, "name"));
            ...
            // populated only when the plugin is actually loaded. Leave them empty here.
            descriptor.display_types = parse_cloud_display_types(item);
            descriptor.entry_package = get_json_string_field(item, "entry_package");
            descriptor.sharing_token = get_json_string_field(item, "sharing_token");
            // main_image.access_url is pre-signed, so the WebView <img> can load it without the
            // Bearer token (content_url, an API path, would 401). Images only; display-only.
            if (item.contains("main_image") && item["main_image"].is_object() && cloud_media_is_image(item["main_image"]))
                descriptor.thumbnail_url = get_json_string_field(item["main_image"], "access_url");
            if (item.contains("changelog") && item["changelog"].is_array()) {
                for (const auto& changelog_item : item["changelog"]) {
                    if (!changelog_item.is_object())
                        continue;

                    PluginChangelog entry;
                    entry.changelog_id = get_json_string_field(changelog_item, "id");
                    entry.plugin_uuid  = get_json_string_field(changelog_item, "plugin_id");
                    if (entry.plugin_uuid.empty())
                        entry.plugin_uuid = uuid;
                    entry.version      = get_json_string_field(changelog_item, "version");
                    entry.changelog    = get_json_string_field(changelog_item, "changelog");
                    entry.created_time = get_json_int64_field(changelog_item, "created_time");

                    descriptor.changelog.push_back(std::move(entry));
                }
                sort_plugin_changelog(descriptor.changelog);
            }

            descriptor.cloud = std::make_optional<CloudPluginState>({
                uuid,
                false,
                false,
                false,
        ...
        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": loaded " << descriptors.size() << " manifests";
    } catch (const std::exception& e) {
        BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": JSON parse error: " << e.what();
        return -1;
    }
...
int OrcaCloudServiceAgent::fetch_mine_manifests_into_descriptors(std::vector<PluginDescriptor>& descriptors)
{
    constexpr int PAGE_LIMIT = 100;
    constexpr int MAX_PAGES  = 1000; // safety backstop against a server that never stops paginating
    ...
    while (true) {
        if (++page_count > MAX_PAGES) {
            BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << ": reached max page count (" << MAX_PAGES
                                       << "), stopping pagination";
            break;
        }

        const std::string path = std::string(ORCA_PLUGINS_MINE) + "?limit=" + std::to_string(PAGE_LIMIT) +
                                 "&offset=" + std::to_string(current_offset) + "&include=changelog";
        ...
        try {
            ...
            // A short (or empty) page means there is nothing left to page through. This is the
            // primary guard against a server that keeps echoing a non-null next_offset forever.
            if (data.size() < static_cast<size_t>(PAGE_LIMIT))
                break;

            // Honor an explicit end-of-pagination signal from the server.
            if (!root.contains("next_offset") || root["next_offset"].is_null())
                break;

            // Require the offset to strictly advance; otherwise a stuck/regressing offset
            // would loop forever.
            const int next_offset = root["next_offset"].get<int>();
            if (next_offset <= current_offset) {
                BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << ": next_offset (" << next_offset
                                           << ") did not advance past " << current_offset << ", stopping pagination";
                break;
            }
...
int OrcaCloudServiceAgent::get_plugin_download_url(const std::string& uuid,
                                                   ...
                                                   std::vector<std::string>& unauthorized)
{
    ...
    auto api_os_type = []() {
        std::string os = platform_os_type();
        if (os == "unknown")
            return std::string{};
        if (os == "macos")
    ...
    const std::string os   = api_os_type();
    const std::string arch = api_architecture();
    std::string path       = ORCA_PLUGIN_DOWNLOAD_URL;
    if (!os.empty())
        path += "?os=" + Http::url_encode(os);
    if (!os.empty() && !arch.empty())
        path += "&arch=" + Http::url_encode(arch);

    json request_item;
    request_item["plugin_id"] = uuid;
    if (!requested_version.empty())
        request_item["version"] = requested_version;

    json request_body;
    request_body["data"] = json::array({request_item});

    int result = http_post(path, request_body.dump(), &response_body, &http_code);
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": " << response_body;
    BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": " << "Status: " << http_code;

    if (result != 0 || http_code != 200) {
        if (http_code == 404)
            BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << " 404, plugin package for " << os << "-" << arch << " not available.";
        else
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << " failed: url=" << path << ", http_code=" << http_code;
        return result != 0 ? result : http_code;
    }
    ...
    try {
        ...
        for (const auto& item : root["data"]) {
            ...
            PluginDownloadData row;
            row.plugin_id     = get_json_string_field(item, "plugin_id");
            row.download_link = get_json_string_field(item, "download_link");
            row.requested_os  = get_json_string_field(item, "requested_os");
            row.returned_os   = get_json_string_field(item, "returned_os");

            if (!row.requested_os.empty() || !row.returned_os.empty())
                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ": artifact fallback requested_os=" << row.requested_os
                                        << ", returned_os=" << row.returned_os;
        ...
        if (root.contains("not_found") && root["not_found"].is_array()) {
            for (const auto& item : root["not_found"]) {
                ...
                PluginDownloadNotFound row;
                row.id     = get_json_string_field(item, "id");
                row.reason = get_json_string_field(item, "reason");
                not_found.push_back(std::move(row));
            }
        }
...
std::string OrcaCloudServiceAgent::get_plugin_url(const std::string& sharing_token) const
{ return cloud_base_url + "/p/" + Http::url_encode(sharing_token); }

int OrcaCloudServiceAgent::subscribe_plugin(const std::string& plugin_uuid)
{
    if (plugin_uuid.empty())
        return -1;

    const std::string request_body = json{{"plugin_id", plugin_uuid}}.dump();
    std::string response_body;
    unsigned int http_code = 0;

    const int result = http_post(ORCA_SUBSCRIBED_PLUGINS, request_body, &response_body, &http_code);
    // Treat an existing subscription as success: the requested cloud state already exists.
    if (http_code == 409)
        return 0;
    if (result != 0 || (http_code != 200 && http_code != 201)) {
...
int OrcaCloudServiceAgent::unsubscribe_plugins(const std::vector<std::string>& plugin_uuids)
{
    if (plugin_uuids.empty())
        return 0;
    ...
    std::string query;
    for (const std::string& plugin_uuid : plugin_uuids) {
        if (plugin_uuid.empty()) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << " failed: empty plugin UUID";
            return -1;
        }

        if (!query.empty())
            query += ",";
        query += Http::url_encode(plugin_uuid);
    }

    const std::string path = std::string(ORCA_UNSUBSCRIBE_PLUGINS) + "?ids=" + query;
    ...
    if (!response_body.empty()) {
        try {
            auto root = json::parse(response_body);
            if (root.contains("not_found") && root["not_found"].is_array() && !root["not_found"].empty()) {
                std::string missing;
                for (const auto& item : root["not_found"]) {
                    if (!item.is_string())
                        continue;
                    if (!missing.empty())
                        missing += ",";
                    missing += item.get<std::string>();
                }
                if (!missing.empty())
                    BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << ": plugin UUIDs not found: " << missing;
            }
        } catch (const std::exception& e) {
            BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << ": response parse error: " << e.what();
        }
    }
                                                   ...
                                                   std::unordered_map<std::string, std::vector<PluginChangelog>>& changelog)
{
    ...
    for (const std::string& uuid : uuids) {
        ...
        const std::string path = std::string(ORCA_PLUGINS_BASE) + "/" + Http::url_encode(uuid) + "/changelog?limit=100";
        std::string response_body;
        unsigned int http_code = 0;

        const int result = http_get(path, &response_body, &http_code);

        if (result != 0 || http_code != 200) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << " failed: plugin_uuid=" << uuid << ", http_code=" << http_code
                                     << ", response=" << response_body;
            if (first_error == 0)
                first_error = http_code != 0 ? static_cast<int>(http_code) : result;
            continue;
        }
        ...
        try {
            ...
            if (!root.contains("data") || !root["data"].is_array()) {
                BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": invalid response format, missing data array for plugin_uuid=" << uuid;
                if (first_error == 0)
                    first_error = -1;
                continue;
            }

            auto& plugin_changelog = changelog[uuid];
            plugin_changelog.clear();
            for (const auto& item : root["data"]) {
                if (!item.is_object())
                    continue;

                PluginChangelog entry;
                entry.changelog_id = get_json_string_field(item, "id");
                entry.plugin_uuid  = get_json_string_field(item, "plugin_id");
                if (entry.plugin_uuid.empty())
                    entry.plugin_uuid = uuid;
                entry.version      = get_json_string_field(item, "version");
                entry.changelog    = get_json_string_field(item, "changelog");
                entry.created_time = get_json_int64_field(item, "created_time");

                plugin_changelog.push_back(std::move(entry));
            }
            sort_plugin_changelog(plugin_changelog);
        } catch (const std::exception& e) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << ": JSON parse error for plugin_uuid=" << uuid << ": " << e.what();
            if (first_error == 0)
                first_error = -1;
            continue;
        }
